...
$(function () {
	...
	$('#userPassword').blur(function () {
        ...
        var passwordForm = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,20}$/;

        if ($('#userPassword').val() != '') {
            if (false === passwordForm.test(userPassword)) {
                $("#sign-check-info-pw").css("color", "red").css('font-size', '8px').text('8~20자 영문대문자, 영문소문자, 숫자, 특수문자를 모두 사용해야합니다.');
    ...
    $('#userPassword2').blur(function () {
        ...
        var passwordForm2 = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,20}$/;
        ...
        if ($('#userPassword').val() != $('#userPassword2').val()) {
            ...
            if ($('#userPassword2').val() != '') {
				...
				if (false === passwordForm2.test(userPassword2)) {
			                $("#sign-check-info-pw2").css("color", "red").css('font-size', '8px').text('8~20자 영문대문자, 영문소문자, 숫자, 특수문자를 모두 사용해야합니다.');
			                userPassword2Check = 0;
			     } else {
			...
			if($('#userNickname').val() !=''){
	        	...
	        	if ($.trim(data) != $('#userNickname').val()) {
						...
						if (false === nicknameForm.test(nickname)) {
							$("#sign-check-info-nick").css("color", "red").css('font-size', '8px').text('영문,한글,숫자사용가능 2~8자리로 가능합니다.');
							userNicknameCheck = 0;
						}else{
